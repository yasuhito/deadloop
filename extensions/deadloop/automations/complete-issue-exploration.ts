...
function persistExplorationWorktreeCleanup(runDir: string, record: JsonObject): void {
  const target = path.join(runDir, "exploration-worktree-cleaned.json");
  const receipt = {
    schemaVersion: 1,
  ...
  if (fs.existsSync(target)) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(receipt)) {
      throw new Error("exploration worktree cleanup receipt is not bound to the attempt");
    }
    return;
  }
...
function publicUnprovableExplorationFailure() {
  return {
    reason: "explorer_state_unprovable",
    explanation: "deadloop could not prove that the read-only explorer left its owned checkout unchanged.",
    recovery: "Inspect the retained workspace and local attempt diagnostics before requesting another exploration.",
  };
}

function persistExplorationDiagnostic(runDir: string, record: JsonObject, detail: string): void {
  const target = path.join(runDir, "exploration-diagnostic.json");
  if (fs.existsSync(target)) {
    const existing = JSON.parse(fs.readFileSync(target, "utf8"));
    if (existing.schemaVersion !== 1 || existing.attemptId !== record.attemptId
      || existing.requestEventId !== record.agentRequest.eventId) {
      throw new Error("exploration diagnostic is not bound to the attempt");
    }
    return;
  }
  const temporary = `${target}.${process.pid}.tmp`;
  ...
  if (record.phase !== "github_persisted" && record.phase !== "workspace_closed") {
    ...
    if (fs.existsSync(explorationOutcomePath(runDir))) {
      ...
      try {
        ...
        persistExplorationDiagnostic(runDir, record, error instanceof Error ? error.message : String(error));
        failure = publicUnprovableExplorationFailure();
      }
