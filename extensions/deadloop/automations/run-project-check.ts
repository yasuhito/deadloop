...
function parseArguments(argv: string[]): ParsedArguments {
  ...
  for (; index < argv.length; index += 1) {
    ...
    if (token === "--quarantine-root") {
      // The durable manifest isolates artifacts beside the worktree; retain this
      // accepted argument so callers using the shared project-check interface work.
      index += 1;
      continue;
    }
...
if (require.main === module) {
  // New internal callers supply a state-owned quarantine root and use the
  // shared implementation. Keep the legacy CLI path for packaged callers
  // that rely on durable manifest recovery and signal re-propagation.
  const runner = process.argv.includes("--quarantine-root") ? projectCheckMain() : main();
  runner.catch((error) => {
