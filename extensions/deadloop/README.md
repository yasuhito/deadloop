...
- `pr-reviewer-driver.cts` consumes one GitHub Agent request per cycle in the order `agent:update-branch`, `agent:implement`, `agent:review`, and handles no-op, pending CI, external review gates, branch-update requests including obsolete ones, one-attempt merge-conflict recovery, reviewer launch, and bounded monitor handoff. Reviewer launch records a complete paginated PR history revision outside the worktree; result dispatch and automatic merge re-observe it and request a fresh review when any commit, diff, comment, submitted review, inline comment, head, or base observation is stale.
...
The host sends a monitor handoff once after launch. Before any retained handoff is redelivered, it re-reads the attempt journal, completion report, and Herdr turn state. A working agent keeps the existing monitor path. A terminal turn without a valid report never receives another Automation-host model prompt: a recognized model billing or access rejection retains the same attempt, workspace, worktree, and agent session and posts one waiting explanation; every other result clears all Agent requests, adds `agent:blocked`, posts one stop explanation, closes only the proven-owned workspace, and retains the worktree and journal. Ambiguous or unreachable runtime evidence is preserved without redelivery. Non-monitor prompts remain one-shot.
...
The host contract the scheduler relies on is small: `pi.exec` for drivers, `pi.sendUserMessage` to hand a `needs_llm` driver prompt to the model, `ctx.isIdle` and `ctx.hasPendingMessages` to hold a tick back while a turn is running, and `ctx.ui.setStatus` / `ctx.ui.notify` for the status line. Both hosts implement all of them, and on both an injected prompt starts a turn. To re-verify after a host upgrade without touching GitHub, load a throwaway extension that calls those APIs from `session_start` and injects a prompt after three seconds, the same delay the scheduler uses; a host that queues the prompt without starting a turn would leave the loop idle while `state.json` still records ticks.
