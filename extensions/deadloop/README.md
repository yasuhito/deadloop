...
Immediately before an automation starts, the host provisions execution supply under `~/.pi/agent/deadloop/code-snapshots/` and `dependency-snapshots/`. Code is archived from the host's loaded commit, so uncommitted working-tree changes are excluded; dependencies are copied from the local installation with hard links, keyed by the committed lock-file SHA-256, without running an installer or using the network. A code generation is reused for the same commit, and dependency generations are shared across commits with identical lock content. Precheck, driver, rendered prompt paths, and attempt records all use this fixed supply; provisioning failure stops before precheck or any mutation-capable driver.
