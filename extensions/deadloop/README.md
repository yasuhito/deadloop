...
The host keeps each queued monitor handoff until the attempt journal reaches GitHub persistence or releases ownership. A bound blocked branch-update report also settles its read-only monitor because that path intentionally retains the workspace and performs no monitor-side mutation. If a host model or API failure ends the monitor turn first, an idle scheduler retries the same bound handoff after one minute; non-monitor prompts remain one-shot.
