...
- `pr-reviewer-driver.cts` consumes one GitHub Agent request per cycle in the order `agent:update-branch`, `agent:implement`, `agent:review`, and handles no-op, pending CI, external review gates, branch-update requests including obsolete ones, one-attempt merge-conflict recovery, reviewer launch, and bounded monitor handoff. Reviewer launch records a complete paginated PR history revision outside the worktree; result dispatch and automatic merge re-observe it and request a fresh review when stale.
