...
- `pr-review-repair-finalize.ts` is the repair worker's only push path. It measures repair breadth before required verification; an oversized repair skips verification, is not pushed, and is handed to a human. Within the size limit, it requires an authenticated passed record bound to the fixed attempt contract and repair commit, reusing only an exact match, then conditionally updates the verified selected branch only while its head equals the validated commit. It writes a finalizer receipt outside the worktree.
...
- Merge-conflict recovery keeps `agent:review` and `agent:reviewing`, records the exact PR-head/base-head attempt in a PR comment, and uses `pr-branch-update-finalize.ts` as the only permitted push path. The finalizer requires the same fixed-contract, output-commit-bound passed record as repair finalization. Stale heads stop without a push; only failed or unsafe updates add `agent:blocked`.
