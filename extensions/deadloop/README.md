...
- `pr-review-repair-dispatch.cts` consumes completed reviewer outcomes. Actionable findings launch one dedicated repair worker; technical reviewer failures retry once for the exact head, while repeated findings add `agent:blocked` with recovery guidance and clear every request label, so a stopped pull request waits on a person and on no agent request. A human-required outcome is a completed review: its result is recorded, a draft becomes ready, and every agent workflow label is removed, so the pull request waits on a person and on no agent request.
