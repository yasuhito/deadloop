...
A `agent:blocked` pull request whose later Agent request follows the authenticated blocked event becomes a review candidate again. Selection and every launch revalidation read that candidate through the same recovery view, so the exact-target comparison still stops a changed revision, state, draft state, or unrelated label, and a pull request that stops qualifying for recovery keeps `agent:blocked` on the live side and fails the comparison. Only a blocked event deadloop itself bound carries the marker this recovery reads.
