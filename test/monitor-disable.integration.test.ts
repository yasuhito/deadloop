...
function mutationRanAfterDisable(monitor: string, useFabricatedState = false): boolean {
  ...
  writeFileSync(
    ...
    JSON.stringify({ lastWriterCodeIdentity: "a".repeat(40), projects: [{ repoPath, githubRepo: "owner/repo", enabledAt: 1, firstEnableAutoMerge: false, firstStartPending: false, lastObservedAutoMerge: false, autoMergeAcknowledged: false, enabled: false }] }),
  );
  ...
  if (useFabricatedState) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ lastWriterCodeIdentity: "a".repeat(40), projects: [{ repoPath, githubRepo: "owner/repo", enabledAt: 1, firstEnableAutoMerge: false, firstStartPending: false, lastObservedAutoMerge: false, autoMergeAcknowledged: false, enabled: true }] }),
    );
  }
