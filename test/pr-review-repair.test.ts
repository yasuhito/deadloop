...
function finalizeWith(
  ...
  return finalizeReviewRepair(
    ...
    {
      ensureVerification: (_args: unknown, _candidate: string, _repositoryId: string, run: (args: string[]) => unknown) => {
        const check = run(["node", "/automation/run-project-check.ts"]) as { status: number; stdout?: string; stderr?: string };
        if (check.status !== 0) throw new Error(check.stderr || check.stdout || "checks failed");
        return check;
      },
...
function finalizeWhileDisabled() {
  ...
  try {
    finalizeReviewRepair(
      {
        repo: "/worktree", projectId: "demo", attemptRecord: "/state/runs/repair/attempt.json", projectRepo: "/repo", githubRepo: "owner/repo", pr: "243",
