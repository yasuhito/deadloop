...
describe("0.8.0 エージェント起動フロー", () => {
  ...
  it("修復試行へ継承したレビュー claim 契約を準備時に固定する", () => {
    ...
    try {
      ...
      const requiredVerification = { repository: "owner/repo", command: "npm test", source: { kind: "repo_policy", location: "deadloop.json" }, baseRevision: "a".repeat(40) };
      const launchInput = { ...input(root, "reviewer"), role: "review-repair", reviewClaim, requiredVerification };
