...
function writePiSession(dir: string, cwd: string): void {
  writeFileSync(path.join(dir, "2026-01-01T00-00-00_pi.jsonl"), [
    JSON.stringify({ type: "session", version: 3, id: "pi-session", timestamp: "2026-01-01T00:00:00.000Z", cwd }),
    "",
    JSON.stringify({
...
describe("normalized model usage records", () => {
  ...
  it("keeps a session whose cwd is outside the attempt checkout unattributed instead of guessing", () => {
    const root = sandbox();
    const sessions = path.join(root, "sessions", "-elsewhere");
    mkdirSync(sessions, { recursive: true });
    writePiSession(sessions, "/other/place");
    ...
    expect(records.every((record: any) => record.role === "unattributed")).toBe(true);
  });
  ...
  it("counts duplicate session records once when parent and artifact copies coexist", () => {
    ...
    copyFileSync(path.join(sessions, "2026-01-01T00-00-00_pi.jsonl"), artifactCopy);
