...
現在の v0 は Pi パッケージ / 拡張として動き、標準実行基盤に Herdr を使います。長期的には Herdr 専用ツールではなく、実行基盤を差し替えられる形に育てます。
...
- 全体像: `README.md`
- Pi 拡張の実体: `extensions/deadloop/README.md`
- Herdr 実行基盤の役割: `docs/herdr-runner.md`
- パッケージ定義: `package.json`
- 自動化プロンプト / 事前確認を触る場合: `extensions/deadloop/automations/`
...
- LLM の出力は信頼できない入力として扱い、構造、revision、実行権限を決定論的に検証してから副作用を許可する。
- 状態遷移、再試行回数、重複排除、所有権解放、後片付けは決定論的かつ冪等にする。不正な出力や終端済み試行を無期限に再処理せず、一度だけ説明可能な停止へ進める。
- LLM の失敗を観測したら、プロンプト調整より先に fixture と回帰テストで再現し、スクリプトまたは TypeScript の境界で吸収できないか確認する。
- 実装、巡回、レビューで、LLM の判断を決定論的処理へ置き換えて状態機械や実行基盤境界を単純化できる箇所を見つけたら、障害発生を待たずにリファクタリング Issue を起票する。Issue には現在の複雑さ、置き換える不変条件、独立して導入できる範囲、回帰テストの受け入れ条件を書く。「整理する」だけの曖昧な Issue にはしない。
...
- 実装、文書、テストは現在の形式と現在の導入状態だけを対象にする。過去のコマンド、環境変数、設定形式、ラベル、報告形式、保存状態のための後方互換処理や移行経路は維持せず、現行経路へ置き換えた時点で削除する。
- 作業中に後方互換性だけを目的とするコード、文書、テストを見つけた場合は、現行経路と安全ゲートから参照されていないことを確認し、関連する不要な処理と一緒に安全に削除する。
- 外部依存が現行経路を実行できる状態かを副作用前に確認する安全ゲートは、後方互換処理とは区別して維持する。

## 安全ルール

- `extensions/deadloop/projects.json` はローカル設定であり、コミットしない。
