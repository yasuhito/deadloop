...
- Reworking the attempt journal's ownership (candidate 3), the observation-once refactor (candidate 4), or unifying verification records and the GitHub adapter (candidate 5). Stage 4 takes only the parts of candidate 5 that the fixture-to-adapter step needs. ADR 0034 takes candidates 2, 3, and 4 after stage 5.
...
2. **Drivers in-process.** `git mv` the two drivers into `src/workflows/`, export `coordinateIssue` / `reviewPullRequest` returning the existing `DriverResult`, replace `process.env` transport with an injected config object and `SCRIPT_DIR = __dirname` with `supply.automationDir`, add the `__dirname` test, replace `driverFile` with `kind`, make the enablement lock async and section-scoped, keep the fixture as an argument. Split each moved driver into `observe` / `plan` / `execute` functions at its existing seams, behaviour unchanged, so that ADR 0034 does not move the files again; making `plan` pure is ADR 0034's work, not this stage's.
...
- `coordinateIssue`, `reviewPullRequest`, and `advanceAttempt` are each composed of exported `observe` / `plan` / `execute` functions split at the existing seams (ADR 0034); their composition is behaviour-identical to the moved code.
