...
| 分類 ID | 最終結果 | 移行先または統合先 |
| --- | --- | --- |
| T350 | 移行済み・Vitest 継続 | 実働担当がいない古いレビュー占有を回収する |
| T351 | 移行済み・Vitest 継続 | 回収したレビューを中断後の再開として扱う |
| T352 | 移行済み・Vitest 継続 | レビュー担当が稼働中の占有を回収しない |
| T353 | 移行済み・Vitest 継続 | ブランチ更新担当の完了を待つ猶予中の占有を回収しない |
| T354 | 移行済み・Vitest 継続 | 終了済みのレビュー担当だけが残る占有を回収する |
| T355 | 移行済み・Vitest 継続 | 意図的に停止されたレビュー占有を回収しない |
| T356 | 移行済み・Vitest 継続 | 未占有の pull request を中断後の再開として扱わない |

「回収後に新しい担当が稼働した占有を重ねて回収しない」は、最初の選定で古い占有を回収し、新しい担当を稼働状態にした次周期に同じ PR を重ねて選ばないことを追加で保証する。
元の `test/pr-reviewer-stale-reclaim.test.ts` 7件と同じ PR・担当状態を決定論的な選定境界へ与え、Cucumber と Vitest が併存した状態で双方の成功を確認した。
受け入れシナリオは外部から観測できる選定結果または再開区分を保証する。元の Vitest は判定用 CLI の引数、検証用データの読込み、JSON 直列化を局所的に診断する価値が残るため、7件とも Vitest 継続へ再分類する。

### 意図的失敗の確認

2026-07-24 に Then assertion を一種類ずつ一時変更し、各変更後に `npm run test:acceptance` が終了状態 1 となることを確認した。

| 対象 | 一時的な変更 | 観測した失敗 |
| --- | --- | --- |
| T350、T354 | 選択番号の期待値を `number + 1` に変更 | 対応する2シナリオだけが失敗し、128件中126件成功・2件失敗を表示 |
| T351 | 中断後の再開区分の期待値を `true` から `false` に変更 | 対応する1シナリオだけが失敗し、127件成功・1件失敗を表示 |
| T352、T353、T355 | 非回収の期待値を `false` から `true` に変更 | 対応する3シナリオだけが失敗し、125件成功・3件失敗を表示 |
| 回収後の重複防止 | 起動数の期待値を `[1, 0]` から `[1, 1]` に変更 | 対応する1シナリオだけが失敗し、127件成功・1件失敗を表示。実値 `[1, 0]` と期待値 `[1, 1]` の差分、および `.feature.md:48` と `.steps.ts:132` の位置を確認 |
| T356 | 通常開始区分の期待値を `false` から `true` に変更 | 対応する1シナリオだけが失敗し、127件成功・1件失敗を表示 |
