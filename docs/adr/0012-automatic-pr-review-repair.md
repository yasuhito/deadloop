...
The repair worker cannot push directly. The deterministic finalizer requires the repair commit to contain the selected PR head, requires a clean worktree, runs the configured checks, immediately re-reads the open PR's branch and head SHA, and uses a normal non-force push to update that exact existing branch. Git fast-forward enforcement prevents it from replacing another head. It never changes labels, creates or merges a PR, closes an issue, or deletes a branch. A stale head stops without an update, comment, or label change so the next cycle can re-evaluate it. A successful push changes the head and starts a new review cycle.
