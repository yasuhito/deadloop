...
For `changes_requested`, deadloop binds a repair attempt to the exact PR head and a deterministic fingerprint of normalized findings. It posts the review summary and structured findings with an HTML attempt marker, then launches one dedicated worker in the existing same-repository PR worktree and branch. `agent:review` and `agent:reviewing` remain present; there is no new label. The findings are the worker's entire contract, and scope widening is prohibited.

The repair worker cannot push directly. The deterministic finalizer requires the repair commit to contain the selected PR head, requires a clean worktree, runs the configured checks, immediately re-reads the open PR's branch and head SHA, and performs a normal non-force push only to that exact existing branch. It never force-pushes, changes labels, creates or merges a PR, closes an issue, or deletes a branch. A stale head stops without push, comment, or label change so the next cycle can re-evaluate it. After a successful push, the finalizer posts a separate repair comment containing the reviewed head, repair commit, commit summary, changed-file statistics, and validation result. It retries that comment once; a repeated comment failure reports that the commit was pushed but blocks the workflow for human recovery rather than silently losing the audit record. A successful push changes the head and starts a new review cycle.
