...
The repair worker cannot push directly. The deterministic finalizer requires the repair commit to contain the selected PR head and requires a clean worktree. Before running configured checks or authorizing a push, it counts changed files from that head through the repair commit with deterministic rename detection and re-reads the finding count from the head-bound repair marker already persisted on GitHub. Automatic repair permits at most five changed files per structured finding and at most twenty changed files overall; exceeding either limit stops without a push and hands the repair to a human. The five-to-one limit catches repairs whose breadth is disproportionate to their stated findings, while the absolute limit bounds a multi-finding repair; both limits are inclusive so a repair exactly at the boundary remains automatic. These initial conservative thresholds are based on the observed regression in qorraq/qorraq-prototype#339, where four findings produced a twenty-nine-file namespace move (7.25 files per finding) and missed a reference that prevented a worker from starting.

Within the size limit, the finalizer requires an authenticated passed verification record bound to the exact output commit and fixed project-check contract. It may reuse only a fully matching authenticated record; otherwise it runs the fixed check and persists a new authenticated record. It immediately re-reads the open PR's branch and head SHA, then updates that exact existing branch with a normal non-force fast-forward push after verifying that the destination still equals the selected PR head. It never changes labels, creates or merges a PR, closes an issue, or deletes a branch. A changed, rewound, or deleted head stops without an update, comment, or label change so the next cycle can re-evaluate it. A successful push changes the head; after the completion handler authenticates that output-bound record again and records the head-bound repair result, it preserves `agent:review` and releases `agent:reviewing`. The next selection records `repair_rereview`, then claims the active review normally. Conflict recovery carries that provenance to its updated head, while a later abandoned active claim remains distinguishable as `stale_reclaim`.
