...
Worker、レビューエージェント、レビュー修正エージェント、ブランチ更新エージェントは V1 完了報告を書く。読み取り側も V1 だけを受け付ける。

この判断は [ADR 0003](./0003-promise-file-contract.md) のプッシュ型ファイル搬送を維持し、試行と対象revisionに結び付いた版付き共通報告を唯一の完了報告とする。
