...
A Worker completion stop caused by unresolved required verification is the sole blocked-attempt exception to this closure rule. The host may close its workspace only after re-reading GitHub and proving that the Issue is open, still has `ready-for-agent`, has `agent:blocked`, has neither `agent:implement` nor `agent:in-progress`, and contains the target-specific fingerprint comment for the exact fixed completion-time verification diagnosis. It then records `github_persisted` before closing the workspace and retains the linked worktree and branch.

All other blocked, human-required, legacy, malformed, missing, launch-failed, and ownership-ambiguous attempts remain visible. A retained attempt suppresses another attempt on the same checkout. Restart reconciliation may close only an already-proven successful V1 attempt or the proven completion-stop exception above and is idempotent.
