# 機能: 作業エージェントを重複なく起動して完了まで監視する

Issue ごとの作業エージェントは専用の作業場所で一度だけ起動し、完了ファイルが確定するまで安全に監視する。
これにより、稼働中や別作業場所の担当を壊さず、活動中の作業を早く終了しない。

## シナリオ: 準備済み Issue の担当には基準ブランチから専用の作業場所を作る

* 前提 作業を開始できる Issue がある
* もし deadloop がその Issue の担当を起動する
* ならば 担当には基準ブランチから Issue 専用の作業場所を作る

## シナリオ: 準備済み Issue の担当は一度だけ起動する

* 前提 作業を開始できる Issue がある
* もし deadloop がその Issue の担当を起動する
* ならば 新しい担当を一人だけ起動する

## シナリオ: 完了済みの同名レビュー担当を片付けてから交代を起動する

* 前提 同じ作業場所に完了済みの同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 完了済みの担当を片付けてから交代を起動する

## シナリオ: 完了済みの同名レビュー担当に代わる担当を一人だけ起動する

* 前提 同じ作業場所に完了済みの同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 一人の交代担当を起動する

## シナリオ: 稼働中の同名レビュー担当がいるときは起動を拒否する

* 前提 同じ作業場所に稼働中の同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 稼働中の同名担当がいるため起動を拒否する

## シナリオ: 稼働中の同名レビュー担当がいるときは新しい担当を起動しない

* 前提 同じ作業場所に稼働中の同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 新しい担当は起動しない

## シナリオ: 重複起動を止めたときも稼働中のレビュー担当を残す

* 前提 同じ作業場所に稼働中の同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 稼働中の担当は残る

## シナリオ: 複数の同名レビュー担当は片付けない

* 前提 同じ作業場所に複数の完了済み同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 候補を特定できない同名担当は片付けない

## シナリオ: 複数の同名レビュー担当がいるときは起動を拒否する

* 前提 同じ作業場所に複数の完了済み同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 複数の同名担当がいるため起動を拒否する

## シナリオ: 複数の同名レビュー担当がいるときは新しい担当を起動しない

* 前提 同じ作業場所に複数の完了済み同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 新しい担当は起動しない

## シナリオ: 別の作業場所にいる同名レビュー担当は片付けない

* 前提 別の作業場所に完了済みの同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 別の作業場所の同名担当は片付けない

## シナリオ: 別の作業場所に同名レビュー担当がいるときは起動を拒否する

* 前提 別の作業場所に完了済みの同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 別の作業場所に同名担当がいるため起動を拒否する

## シナリオ: 別の作業場所に同名レビュー担当がいるときは新しい担当を起動しない

* 前提 別の作業場所に完了済みの同名レビュー担当がいる
* もし deadloop が pull request のレビュー担当を起動する
* ならば 新しい担当は起動しない

## シナリオ: 起動した担当を完了ファイルの監視へ引き渡す

* 前提 作業を開始できる Issue が選ばれている
* もし deadloop が選ばれた Issue の作業を開始する
* ならば その Issue は完了ファイルの監視対象になる

## シナリオ: 報告要求後に最近活動した担当は監視を続ける

* 前提 完了ファイルを求めた後に担当の最近の活動がある
* もし deadloop が担当の監視状態を判断する
* ならば 担当の監視を続ける

## シナリオ: 完了ファイルを求めた後の猶予中は監視を続ける

* 前提 完了ファイルを求めてから猶予時間内である
* もし deadloop が担当の監視状態を判断する
* ならば 担当の監視を続ける

## シナリオ: 完了ファイルがない担当には終了前に報告を求める

* 前提 活動を終えた担当の完了ファイルがない
* もし deadloop が担当の監視状態を判断する
* ならば 担当に完了ファイルを書くよう求める

## シナリオ: 活動停止と猶予経過を確認した後だけ終了を許す

* 前提 担当の活動停止と報告要求後の猶予経過を確認できる
* もし deadloop が担当の監視状態を判断する
* ならば 担当画面の終了を許す

## シナリオ: 終了判断に必要な観測がなければ追加確認する

* 前提 報告要求後の猶予は過ぎたが担当画面の観測がない
* もし deadloop が担当の監視状態を判断する
* ならば 終了前に不足した観測を集める

## シナリオアウトライン: 完了ファイルが完成したら担当の状態にかかわらず監視を終える

* 前提 <状態>の担当が完了ファイルを書き終えている
* もし deadloop が担当の監視状態を判断する
* ならば 完了ファイルに従って監視を終える

### 例:

  | 状態 |
  | 稼働中 |
  | 活動終了 |
