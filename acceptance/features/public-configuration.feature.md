...
## シナリオ: 共有方針はローカルで省略した Reviewer の種別とモデルを補う

* 前提 Reviewer の種別とモデルを含む共有方針と空のローカル設定がある
* もし Reviewer の起動を要求する
* ならば Reviewer は共有方針の種別とモデルで起動する

## シナリオ: ローカルの Reviewer の種別とモデルは共有方針より優先する

* 前提 Reviewer の種別とモデルが異なる共有方針とローカル設定がある
* もし Reviewer の起動を要求する
* ならば Reviewer はローカルの種別とモデルで起動する
...
* 前提 CI 代替コマンドを含み、有効化を省略したローカル設定がある
* かつ CI が課金制限によるインフラ障害で失敗している
* もし CI 代替実行を要求する
* ならば CI 代替コマンドは起動されない
