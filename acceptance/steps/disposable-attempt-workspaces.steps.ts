...
When("deadloop starts an automation cycle", async function (this: World) {
  const project = normalizeProject({ workerModel: "test-model", reviewerModel: "review-model", id: "demo", repoPath: "/repo", githubRepo: "owner/repo", automations: [{ id: "a", name: "a", driverFile: "driver.cts" }] });
  try {
    await runScheduledAutomation(project, project.automations[0], 1, { automations: {} }, {
      ...
      resolveAutomationFileInDir: () => ({ requested: "", resolved: "", found: false }),
      runDriver: async () => (this.mutationCount!++, { code: 0 }),
      saveState: () => { this.mutationCount!++; },
    });
  } catch {}
});
