...
**作業権限 (Work authority)**:
あるPRの現在の作業を、どの試行が持っているかという状態。GitHubのAgent要求、claim、ラベルと、ローカルの試行記録、実行基盤の観測を照合して決まる。証明できない場合は作業権限を与えず`agent:blocked`にする。停止した試行の作業権限は、新しいAgent要求が待っている間に限り、その試行が現在のheadに対して何もできないことを確認したうえで解放する。解放しても試行の記録とworktreeは証拠として残り、失われるのは所有の主張だけである。
