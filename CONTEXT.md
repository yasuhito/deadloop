...
GitHub上で、対象のIssueまたはPRに対して特定の仕事を求めるWorkflow上の意思。`agent:implement`、`agent:review`、`agent:update-branch`などのラベル追加イベントで表す。PR要求の消費は、そのラベル追加イベントidに結び付けて記録する。同時更新で新しい要求が加わった場合は事後に検知して要求ラベルを戻す。停止中の仕事に同じ役割の新しいAgent要求を追加した場合は、実行基盤で古い試行の停止を確認してから新しい試行を開始する。ローカルコマンドによる試行の再開または放棄とは区別する。
_Avoid_: 現在のラベル一覧だけ、ローカルの試行記録、`/deadloop-abandon-attempt`

**要求消費 (Request consumption)**:
PRのAgent要求ラベルを`agent:in-progress`へ置き換え、その要求イベントidを試行記録へ残す遷移。GitHubへ所有権コメントは書かず、試行記録も許可の根拠にはしない。同時更新による重複消費は、最新の要求イベントidを再取得して事後に検知・修復する。破壊的な書き込みの安全性は対象headに対するcompare-and-swap、試行の生存確認は実行基盤だけが担う。
