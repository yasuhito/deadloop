...
PRへ`agent:in-progress`を非破壊的に追加し、開始時に存在した他の管理対象ラベルを個別に外した後、選んだAgent要求ラベルを最後に外して、その要求イベントidを試行記録へ残す遷移。選んだラベルの削除がHTTP 200で確認できた時点だけを消費の成立点とする。GitHubへ所有権コメントは書かず、試行記録も許可の根拠にはしない。同時更新は各書き込みの前後で全役割の最新要求イベントidと現在のラベルを再取得して検知し、新しい要求は残すが取り消された要求は戻さない。破壊的な書き込みの安全性は対象headに対するcompare-and-swap、試行の生存確認は実行基盤だけが担う。
