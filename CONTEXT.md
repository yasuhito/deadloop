...
GitHub上で、対象のIssueまたはPRに対して特定の仕事を求めるWorkflow上の意思。`agent:implement`、`agent:review`、`agent:update-branch`などのラベル追加イベントで表す。PR要求の消費は、そのラベル追加イベントidに結び付けて記録する。同時更新で新しい要求が加わった場合は事後に検知して要求ラベルを残す。停止中の仕事に同じ役割の新しいAgent要求を追加した場合は、実行基盤で古い試行の停止を確認してから新しい試行を開始する。ローカルコマンドによる試行の再開または放棄とは区別する。
_Avoid_: 現在のラベル一覧だけ、ローカルの試行記録、`/deadloop-abandon-attempt`

**要求消費 (Request consumption)**:
PRへ`agent:in-progress`を非破壊的に追加し、開始時に存在した他の管理対象ラベルを個別に外した後、選んだAgent要求ラベルを最後に外して、その要求イベントidを試行記録へ残す遷移。選んだラベルの削除がHTTP 200で確認できた時点だけを消費の成立点とする。GitHubへ所有権コメントは書かず、試行記録も許可の根拠にはしない。同時更新は各書き込みの前後で全役割の最新要求イベントidと現在のラベルを再取得して検知する。現在もラベルがある新しい要求はそのまま残し、ラベルがない世代や状態が曖昧な世代は推測で戻さず、`agent:in-progress`と準備済みの試行記録を残して停止する。破壊的な書き込みの安全性は対象headに対するcompare-and-swap、試行の生存確認は実行基盤だけが担う。
_Avoid_: 作業権限、所有権コメント、コメント時刻による排他制御
