...
GitHub上で、対象のIssueまたはPRに対して特定の仕事を求めるWorkflow上の意思。`agent:implement`、`agent:review`、`agent:update-branch`などのラベル追加イベントで表す。IssueとPRで同じ意味を持ち、どの試行が仕事を所有するかは表さない。実行中に追加した要求は、現在の試行とは別の次の試行を求め、役割ごとに最大一つ待機する。停止中に役割に対応する新しいAgent要求を追加した場合も、新しい試行を求める。ローカルの試行記録とclaim commentはAgent要求ではなく、仕事を始める許可も与えない。
_Avoid_: 作業権限、claim、現在のラベル一覧だけ、ローカルの試行記録、`/deadloop-abandon-attempt`

**Agent要求の消費 (Agent request consumption)**:
一意に記録された試行が、起動前に一つのAgent要求を引き受け、待機中の要求から実行中の仕事へ移すこと。消費前に要求ラベルを外すと要求の取消になる。消費後にラベルを外しても、既に動いている試行の停止にはならない。
_Avoid_: エージェント起動後の消費、試行を記録する前の消費、実行中の試行の取消
