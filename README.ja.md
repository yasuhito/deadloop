...
1. **作業を依頼する** — `agent:implement` を付けると実装を依頼でき、`agent:explore` を付けると読み取り専用の調査を依頼できます。両方が待機している場合は調査を優先します。`ready-for-agent` は任意のトリアージ情報です。deadloop が選択した要求世代を消費する前に要求ラベルを外すと、依頼を取り消せます。
...
3. **完了または対応する** — 承認された PR は、自動マージが無効なら ready へ変わり agent 系ワークフローラベルが外れます。有効ならマージされます。`ready-for-human` は Issue の分類用ラベルであり、PR には付きません。`agent:blocked` が付くとループは止まります。Issue の調査が失敗した場合や安全性を証明できない場合は、停止より前の要求を外します。停止より後に追加された要求は復旧用の依頼として残り、新しい試行の対象になります。コメントに記載された原因を解消し、次に実行したい役割の要求ラベルを追加してください。`agent:blocked` はその試行が始まった時点で消えます。
