...
Issue 実装の Worker、探索、PR レビュー、レビュー修復、ブランチ更新の各試行監視には、Automation host のモデルを使いません。実行基盤が作業中と報告している間は出力が途切れても継続し、設定済みの24時間上限を実作業時間へ適用します。完了報告がないままターンが終わっても、会話で完了報告を催促しません。Worker の完了報告に対しては、必須検証から宛先固定の push、レビュー依頼つきドラフト PR 作成、結果の永続化までを一つの決定論的な連鎖として実行し、探索の結果は次の Issue アクションとともに検証・永続化されます。この必須検証が失敗した場合、host は同じ入力（リポジトリ、対象コミット、コマンド、ソース、ベースリビジョン）のうちは毎 tick コマンドを再実行せず、exit code と log の末尾を含む fingerprint 付きの冪等コメントを Issue に 1 回だけ投稿します。入力が変わると新たに実行して記録を置き換えます。
