...
レビューまたは修復の停止を実行基盤が確認したら、記録の前に完了報告をもう一度読み直し、その停止を試行と選定時の PR head に結び付けて PR 上に表示します。修復の場合は、その契約となった必須指摘のキーでも結び付けます。`ENOSPC` や `EDQUOT` のせいで完了報告ファイル自体を読めなかった停止は容量不足停止として記録し、空き容量を確保する復旧手順を案内します。pane の文字列だけではこの原因をつけません。どちらの失敗も上限付きの技術的再試行に入らず、自動で再試行しません。停止した修復は `agent:implement` を付け直すだけで再開できます。公開されたマーカーと保持された試行記録が証明できる同一の指摘契約だけを再度起動し、停止時の worktree は保持したまま、新しい claim が永続化されるまで `agent:blocked` を維持します。
