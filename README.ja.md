...
最終処理では、まず修正範囲を測定します。上限を超えた場合は、必須検証を実行せず、push せずに人間へ引き渡します。

修正が上限内の場合は、試行に固定した必須検証契約と修正コミットに結び付く成功記録を要求します。完全に一致する記録だけを再利用し、対象ブランチの先頭が検証済みコミットと一致する場合だけ、そのブランチを不可分に更新します。
