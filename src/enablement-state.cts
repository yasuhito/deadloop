...
function normalizeEnablementStateValue(value: unknown): EnablementStateValue | null {
  ...
  if (typeof state.lastWriterCodeIdentity !== "string" || !/^[0-9a-f]{40}$/i.test(state.lastWriterCodeIdentity)) return null;
  ...
  return {
    projects: normalized,
    lastWriterCodeIdentity: state.lastWriterCodeIdentity.toLowerCase(),
  };
}
