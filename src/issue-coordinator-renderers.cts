...
function renderIssuePlanningComment(input: IssuePlanningCommentInput): string {
  return [
    ...
    "3. When order matters, chain the implementation issues with GitHub's native dependency: on each issue, add a \"Blocked by\" relationship pointing at the issue that must close first.",
    "4. Note the number of the implementation issue you created or split out, then re-queue that issue safely (replace `123` below):",
    "```bash",
