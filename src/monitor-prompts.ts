...
- If that command returns action=stale_history, do not hand off as reviewed. It releases ${input.reviewingLabel} while retaining ${input.reviewLabel}; then close the reviewer workspace with \`${renderWorkspaceCompletion(input, [input.reviewLabel])}\`.
