...
function evaluateCompletionPersistence(input){
 ...
 if(r.role==="reviewer"&&r.result.outcome==="human_required")return{action:"preserve",reason:"human_required"};
 ...
 if(r.role==="worker"){
  ...
  const p=github.reviewPersistence;const expected=context["reviewerExpectedLabels"];const managed=new Set(context["reviewerManagedLabels"]||expected||[]);ok=sha(github.headSha,record.inputRevision.head)&&Array.isArray(expected)&&seteq(github.labels.filter(x=>managed.has(x)),expected)&&!!p&&bound(p,record)&&sha(p.headSha,record.inputRevision.head)&&marker(p.marker,record,r.result.outcome)&& (r.result.outcome!=="changes_requested"||(p.boundedRepairAttemptMarked&&findings(p.findings,r.result.requiredFindings||[])));
 } else {
