...
function parseAttemptRecord(value) {
  ...
  if (value.phase === "authority_released") {
    ...
    if (!["owner_absent", "terminal_missing_report", "runtime_timeout", "never_launched"].includes(value.authorityRelease.reason)) throw new Error("Invalid attempt record: authorityRelease.reason is invalid");
